...
/* 1.- Crear un trigger que cada vez que se inserte una fila en la tabla VENTAS,
       ...
       CREATE TRIGGER TR_ACTVENTASMENSUALES
       ON VENTAS AFTER INSERT
       AS
       DECLARE @_VENDEDORCOD CHAR(3),
			   @_VENTAFECHA DATE,
			   @_VENTAmONTO MONEY
	   SELECT  @_VENDEDORCOD=inserted.vendedorCodigo,@_VENTAFECHA=inserted.ventaFecha,@_VENTAmONTO=INSERTED.ventaMonto
	   FROM inserted
	   IF EXISTS (SELECT MONTH(I.ventaFecha), YEAR(I.ventaFecha) FROM VENTASMENSUALES VM, inserted I 
				  WHERE VM.año=YEAR(I.ventaFecha)AND VM.mes=MONTH(I.ventaFecha)AND VM.vendedorCodigo=I.vendedorCodigo)
	   BEGIN 
	   UPDATE VENTASMENSUALES SET totalVentas=totalVentas+@_VENTAmONTO
	   WHERE año=YEAR(@_VENTAFECHA)AND mes=MONTH(@_VENTAfECHA) AND vendedorCodigo=@_VENDEDORCOD
	   END 
	   ELSE
	   BEGIN
	   INSERT INTO VENTASMENSUALES VALUES(@_VENDEDORCOD,MONTH(@_VENTAFECHA),YEAR(@_VENTAFECHA),@_VENTAmONTO)
	   END 
	...
	insert into VENTAS values('vt7','2015-1-04',2000,'c02','v01')
...
/* 1.- Crear un PA que reciba como parámetro el nombre de un vendedor y 
	...
	alter proc sp_TotalVentasVendedor @_vendedorNombre varchar(100)
	as
	select TotalVentas=count(ventaCodigo),Ventas=SUM(ventaMonto),vendedorApellido, vendedorNombre  from  VENDEDORES v inner join VENTAS ve 
...
/* 2.- Crear un PA que despliegue el total de ventas realizado por todos los vendedores en un mes
	...
	alter  proc sp_TotalVentasMes @_mes int, @_año int
	as 
	...
	exec sp_TotalVentasMes 14,2013
