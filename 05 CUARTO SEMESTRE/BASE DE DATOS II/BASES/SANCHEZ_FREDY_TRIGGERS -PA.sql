...
/* 1.- Crear un trigger que cada vez que se inserte una fila en la tabla VENTAS,
	...
	create trigger tr_ActTotalVentasMensualesA
	on VENTAS after insert
	as
	declare @_ventaCodigo char(3), @_ventaFecha date, @_ventaMonto money, @_clienteCodigo char(3), 
	        @_vendedorCodigo char(3)
	...
	begin
	insert into VENTASMENSUALES values (@_vendedorCodigo,MONTH(@_ventaFecha),YEAR(@_ventaFecha),@_ventaMonto)
	end
	
	insert into VENTAS values('vt7','2017-01-01',2000,'c02','v02')
...
/* 2.- Crear un trigger que cuando se borre una fila de la tabla VENTAS,
	...
	create trigger tr_ActTotalVentasMensualesB
	on VENTAS after insert
	as
	declare @_ventaCodigo char(3), @_ventaFecha date, @_ventaMonto money, @_clienteCodigo char(3), 
	        @_vendedorCodigo char(3)
    ...
    @_clienteCodigo=deleted.clienteCodigo, @_vendedorCodigo=deleted.vendedorCodigo from deleted
	update ventasmensuales set totalventas = totalventas - @_ventaMonto
	where mes=month(@_ventaFecha) and año=year(@_ventaFecha) and vendedorcodigo=@_vendedorCodigo 
...
/* 3.- Crear un trigger que cuando se actualice una fila de la tabla VENTAS,
	...
	update  VENTAS set ventaMonto=1000 where ventaCodigo='vt1'
...
/* 2.- Crear un PA que despliegue el total de ventas realizado por todos los vendedores en un mes
	...
	as 
	select TotalVentas=SUM(v.ventaMonto),Mes=MONTH(ventaFecha),Año=YEAR(ventaFecha) from  VENTAS v
...
/* 3.- Crear un PA que despliegue el nombre del vendedor, la fecha y el monto de todas las compras
	...
	exec sp_Selecciones
/* 4.- Crear un procedimiento almacenado que despliegue el total de ventas realizadas en la empresa*/
	create proc sp_TotalVentas
	as
	select TotalVentas=SUM(totalVentas) from VENTASMENSUALES
