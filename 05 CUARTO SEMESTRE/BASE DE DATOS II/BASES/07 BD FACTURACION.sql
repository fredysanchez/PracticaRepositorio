...
CREATE DATABASE BDFACTURACION_1
GO
USE BDFACTURACION_1
GO 
...
/*Crear un trigger que cada vez que se inserte una fila en Facturas y el tipoFactura sea "crédito",
  ...
  as
  declare @nf char(3),@vf money, @ff  date , @cc char(3), @tf varchar(10)
  select @nf=inserted.nroFact,@vf=inserted.valorFact,@ff=inserted.fechaFact,@cc= inserted.codCLi,@tf=inserted.tipoFactura
  from inserted
  if exists (select c.codCli from CREDITOS c, inserted i
			  where c.codCli=i.codCLi and c.fechaCredito=i.fechaFact )
  begin 
  update CREDITOS set cantidadCredito=cantidadCredito+@vf
  where codCli=@cc and fechaCredito=@ff and @tf='Credito'
  end
  else
  begin
  insert into CREDITOS select @ff,@vf,@cc where @tf='Credito'
  end 
 ...
 disable trigger tr_ActualizarCreditos
...
insert into FACTURAS values('025',89,'2014-02-25','C02','Credito')
...
exec sp_InformacionProducto 'Atun'
...
select  @_nroFact=inserted.nroFact,
		...
		@_codCli=inserted.codCLi
		from inserted
     if exists(SELECT valorFact FROM FACTURAS Ft inner join DETALLES Dt on ft.codCLi=@_codCli where valorFact between 100 and 199)
     begin
		update CLIENTES set descuento=0.05 where @_codCli=codCLi
	 end
	 else if exists ((SELECT valorFact FROM FACTURAS Ft inner join DETALLES Dt on ft.codCLi=@_codCli where valorFact>200)) 
	 begin
		update CLIENTES set descuento=0.10 where @_codCli=codCLi
	 end
	 else 
	 begin 
		 update CLIENTES set descuento=0.0 where @_codCli=codCLi
	 end
	 
	 insert into FACTURAS values('017',2000,'2011-03-23','C05','Credito')
